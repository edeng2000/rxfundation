...
WindowsLoadLib::WindowsLoadLib(){
	if (WindowsLoadLib::countinstance==0){
		...
		//WTS LIB
		WindowsLoadLib::wtsdll=LoadLibrary(_T("Wtsapi32.dll"));
		...
		//SHCORE LIB
		WindowsLoadLib::shcoredll = LoadLibrary(_T("Shcore.dll"));
		...
		//USER32 LIB
		WindowsLoadLib::user32dll = LoadLibrary(_T("User32.dll"));
		...
		//SAS LIB
		WindowsLoadLib::sasdll = LoadLibrary(_T("sas.dll"));
		if (WindowsLoadLib::sasdll){
			WindowsLoadLib::sendSasFunc = (ISendSas)GetProcAddress(sasdll, "SendSAS");
		}else{
			WindowsLoadLib::sasdll = LoadLibrary(_T("native\\sas.dll"));
			if (WindowsLoadLib::sasdll){
				WindowsLoadLib::sendSasFunc = (ISendSas)GetProcAddress(sasdll, "SendSAS");
			}
		}
		...
		//WINSTA LIB
		WindowsLoadLib::winstadll = LoadLibrary(_T("winsta.dll"));
